...
export class PathVisualizer extends Component {
  ...
  handleClick(element, state, index) {
    if (state === "down") {
      ...
      if (this.selectedBrush && this.mousePressed) {
        if (this.selectedBrush === "wall" || this.selectedBrush === "empty") {
  ...
  solve(algorithm) {
    ...
    if (this.calculating === false) {
      ...
      this.text = (
        <>
          <span className="left">{Math.round(timeTaken * 100) / 100} ms</span>

          <i className="material-icons right hide-on-med-and-down">timer</i>
        </>
      );
      this.refresh();
  ...
  animate(visited, path) {
    if (visited.length !== 0) {
      ...
      var iterations = 0;
      function animate() {
        const el = document.getElementById(`node-${visited[iterations]}`);
  ...
  animatePath(path) {
    if (path.length !== 0) {
      ...
      function animate() {
        ...
        if (iterations === path.length - 2) clearInterval(this.pathInterval);
